...
>(({ className, children, ...props }, ref) => {
  const { value, onValueChange, activeIndex, realValues } = useMultiSelect();
      ...
      {value.map((item, index) => (
        ...
        >
          <span className="text-xs">
            {realValues.find((value) => String(value.id) === item).name}
          </span>
          <button
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <CommandPrimitive.Input
      ...
      className={cn(
        "bg-transparent outline-none border placeholder:text-muted-foreground flex-1",
        className,
